...
mainLoop :: Socket -> IO ()
mainLoop sock = do
  (c1, _) <- accept sock; h1 <- socketToHandle c1 ReadWriteMode
  hSetBuffering h1 LineBuffering; hSetEncoding h1 utf8
  hPutStrLn h1 "PLAYER: X"; hPutStrLn h1 "MSG: Welcome player (X). Waiting for opponent..."
  (c2, _) <- accept sock; h2 <- socketToHandle c2 ReadWriteMode
  hSetBuffering h2 LineBuffering; hSetEncoding h2 utf8
  hPutStrLn h2 "PLAYER: O"; hPutStrLn h2 "MSG: Welcome player (O)."
...
playerLoop hMy hOther gv me mMine mOther = session `catch` handler``
  where
    session = do
      ...
      when ended $ do
        ...
        if mine && other
          then do
            when (me == X) $ do
              _ <- tryTakeMVar mMine; _ <- tryTakeMVar mOther
              _ <- swapMVar gv initialGame
              sendGame hMy gv; sendGame hOther gv
    ...
    loop = do
      ln <- hGetLine hMy
      if ln == "GAME_OVER_ACK" then return True
          ...
          modifyMVar_ gv $ \g ->
            if gameStatus g /= Running then return g
              ...
              then hPutStrLn hMy "MSG: Not your turn!" >> return g
                ...
                if not ok then hPutStrLn hMy "MSG: Invalid move." >> return g
...
applyMove g me ln =
  case words ln of
    [rS,cS] -> case (reads rS, reads cS) of
      ([(r,"")],[(c,"")]) ->
        let r0 = r - 1; c0 = c - 1
...
updateGameStatus g p r c =
  case findWin (board g) p (r,c) of
    ...
    Nothing    -> if all (notElem Empty) (board g) then g { gameStatus = Draw } else g
...
checkDir b p (r,c) (dr,dc) =
  let go rr cc drr dcc acc =
        ...
        in if r'<0 || r'>=boardSize || c'<0 || c'>=boardSize || b !! r' !! c' /= Taken p
             then acc else go r' c' drr dcc ((r',c'):acc)
      neg = go r c (-dr) (-dc) []
      pos = reverse (go r c dr dc [])
      full = neg ++ [(r,c)] ++ pos
  in if length full < winCondition then Nothing
